...
func NewSessionID(signingKey string) (SessionID, error) {
	if len(signingKey) == 0 {
		return InvalidSessionID, errors.New("signingKey length should not be 0")
	}
	...
	h.Write(id)
	signiture := h.Sum(nil)
	signing := append(id, signiture...)

	return SessionID(base64.URLEncoding.EncodeToString(signing)), nil
}
